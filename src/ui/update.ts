...
export type UiUpdateReturn = [UiModel, readonly Command.Command<UiMessage>[]];
...
): readonly [Ui.Animation.Model, readonly Command.Command<UiMessage>[]] => {
  ...
  M.value(message).pipe(
    ...
    M.tagsExhaustive({
      ...
      ClickedDismissAllToasts: () => {
        const [nextToastDemo, toastCommands] = Toast.dismissAll(
          model.toastDemo
        );

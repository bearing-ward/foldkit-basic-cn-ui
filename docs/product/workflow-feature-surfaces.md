...
| --- | --- | --- | --- |
| New component authoring | `/docs/new-component` | `src/newComponentAuthoring.ts`; routed by `src/main.ts`; listed in `src/docsView.ts`; scaffold checklist commands surfaced in the UI: `bun run build:registry`, `bun run check:registry`, `bun run typecheck`, and `bun run test` | `src/newComponentAuthoring.scene.test.ts`; focused route coverage in `src/main.scene.test.ts` |
| Theme playground | `/docs/theme-playground` | `src/themePlayground.ts`; routed by `src/main.ts`; listed in `src/docsView.ts` | `src/themePlayground.scene.test.ts` |
...
| `bun run test src/main.scene.test.ts src/newComponentAuthoring.scene.test.ts src/themePlayground.scene.test.ts` | Docs app routing, New component authoring checklist, and Theme playground behavior |

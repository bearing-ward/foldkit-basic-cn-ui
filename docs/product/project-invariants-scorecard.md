...
| `P5_EXAMPLE_PARITY` | PARTIAL | 4 | 1 - needs complete row verification | origin-content parity review; example source; example tests check | `bun run check:registry` and `bun run test` passed on 2026-06-22 @ 81de3575 | README still records visual drift and missing edge-case behavior. | Keep unchecked rows in `docs/product/origin-content-parity-review.md` until side-by-side evidence agrees. | 2026-06-22 @ 81de3575 |
| `P6_VISUAL_PARITY` | PARTIAL | 4 | 1 - needs full public-surface visual evidence | origin visual parity coverage script; fixtures and references; origin visual parity test command | `bun run origin:parity:coverage` and `bun run check:registry` passed on 2026-06-22 @ 81de3575 | Coverage fixtures exist, but README still warns about visual drift and follow-up passes. | Expand and refresh fixtures when origin-backed visual/API surfaces change. | 2026-06-22 @ 81de3575 |
...
| `P10_ORIGIN_API_PARITY` | PARTIAL | 4 | 1 - needs full public API parity evidence | plan 019 policy/result; `check:no-component-classname-api`; upstream snapshots/contracts; generated registry JSON; focused and full test commands | `bun run check:registry` and `bun run test` passed on 2026-06-22 @ 81de3575 | Plan 019 source and guard behavior are present, but broader origin API parity still needs ongoing component-by-component evidence across the intended public surface. | Expand future contract coverage and focused shadcn/Base UI tests when public origin-backed APIs change. | 2026-06-22 @ 81de3575 |

...
export const UpdatedWebhookUrl = m("UpdatedWebhookUrl", { value: S.String });

export const Message = S.Union([UpdatedWebhookUrl]);
...
export const update = (
  ...
  M.value(message).pipe(
    ...
    M.tagsExhaustive({
      UpdatedWebhookUrl: ({ value }) => [
        evo(model, { webhookUrl: () => value }),
        [],
      ],
    })
  );
...
export const view = Submodel.defineView<Model, Message>((model): Html => {
  ...
  return field(h, {
    id: "webhook-url",
    label: "Webhook URL",
    labelAddon: Badge.view<Message>({ label: "Beta", variant: "Secondary" }),
    value: model.webhookUrl,
    onInput: (value) => UpdatedWebhookUrl({ value }),
    type: "url",
  });
});

  ...
  return h.div(
    ...
    [
      h.div(
        [h.Class("space-y-1")],
        [
          h.h2([h.Class("text-lg font-semibold text-gray-950")], ["Verify your login"]),
          h.p([h.Class("text-sm text-gray-600")], ["Enter the verification code we sent to your email address: m@example.com."]),
        ]
      ),
      h.div([h.Class("flex justify-center")], [otpView(model)]),
      
      h.button([h.Type("submit"), h.Class("rounded-md bg-gray-950 px-3 py-2 text-sm font-medium text-white")], ["Verify"]),

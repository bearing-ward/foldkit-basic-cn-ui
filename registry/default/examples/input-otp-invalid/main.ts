  ...
  return h.div(
    ...
    [
      h.div(
        [h.Class("space-y-1")],
        [
          h.h2([h.Class("text-lg font-semibold text-gray-950")], ["Invalid"]),
          h.p([h.Class("text-sm text-gray-600")], ["Please enter a valid one-time password."]),
        ]
      ),
      h.div([h.Class("flex justify-center")], [otpView(model)]),
      
      
      h.p([h.Class("text-sm text-red-600")], ["Please enter a valid one-time password."]),
    ]
  );
});
